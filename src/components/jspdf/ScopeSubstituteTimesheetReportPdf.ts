...
interface ScopeSubstituteTimesheetReportPdf {
    data: any[];
    setPdfBlob: (blob: Blob) => void;
}


const ScopeSubstituteTimesheetReportPdf: React.FC<ScopeSubstituteTimesheetReportPdf> = ({ data, setPdfBlob }) => {
        ...
        Object.keys(groupedByPersonID).forEach((personID, personIndex) => {
            ...
            Object.keys(siteData).forEach((siteID, siteIndex) => {
                ...
                siteEmployees.forEach((item: any, employeeIndex: any) => {
                    ...
                    doc.setFontSize(18);
                    // Title
                    doc.text("SCOPE Substitute - Time Sheet", margin, y);
                    y += 8;
                    ...
                    doc.text(`${String(item.Max_Hours ? item.Max_Hours : '')} a week`, margin + 22, y);
                    ...
                    autoTable(doc, {
                        ...
                        head: [
                            ['', 'Date', 'In', 'Out', 'Code', '#Hours', 'Supervisor Sign'],
                        ],
                        body: programHours,
                        theme: 'grid',
    ...
    const addBottomTable = (doc: jsPDF) => {
        ...
        const rightTableBody = [
            ...
            ['1', 'Assigned role of Site Director \n(full program session)', '3', 'Assigned to a program \n(different district)'],
            ['2', 'Assigned to another program \n(same district)', '4', 'Perm sub assigned to program outside \nof cluster'],
        ];
...
export default memo(ScopeSubstituteTimesheetReportPdf);