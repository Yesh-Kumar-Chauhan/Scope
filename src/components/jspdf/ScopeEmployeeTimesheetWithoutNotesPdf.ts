...
interface ScopeEmployeeTimesheetWithoutNotesPdf {
    data: any[];
    setPdfBlob: (blob: Blob) => void;
}


const ScopeEmployeeTimesheetWithoutNotesPdf: React.FC<ScopeEmployeeTimesheetWithoutNotesPdf> = ({ data, setPdfBlob }) => {
        ...
        Object.keys(groupedByPersonID).forEach((personID, personIndex) => {
            ...
            Object.keys(siteData).forEach((siteID, siteIndex) => {
                ...
                siteEmployees.forEach((item: any, employeeIndex: any) => {
                    ...
                    doc.text(`${item.FIRSTNAME}, ${item.LASTNAME}`, margin + 20, y);
...
export default memo(ScopeEmployeeTimesheetWithoutNotesPdf);