...
interface Scope11TimesheetPdf {
    data: any[];
    setPdfBlob: (blob: Blob) => void;
}


const Scope11TimesheetPdf: React.FC<Scope11TimesheetPdf> = ({ data, setPdfBlob }) => {
        ...
        Object.keys(groupedByPersonID).forEach((personID, personIndex) => {
            ...
            Object.keys(siteData).forEach((siteID, siteIndex) => {
                ...
                siteEmployees.forEach((item: any, employeeIndex: any) => {
                    ...
                    doc.setFontSize(18);
                    // Title
                    doc.text("SCOPE 1:1 - Time Sheet", margin, y);
                    y += 8;
...
export default memo(Scope11TimesheetPdf);